...
1. Update the tests in the database
    ```
    cd ~/db-devops-tools/liquibase
    liquibase --contexts=test update
    ```
...
The helper procedure, "delete_added_customers", is used to clean up the customer records generated by the function.  In this case you are calling it before any tests are run to make sure any old test customers have been removed and you start with a fresh environment.

1. Git push
1. Check results
   This time it passed.  The before_all setup procedure sets up the environment so that it only contains the two "test" records created by the liquibase insert step.
...
## After all
In **SQL Developer Web** Execute the following query
```
