...
```
oci db autonomous-database get --autonomous-database-id $DB_OCID --query 'data."connection-urls"."sql-dev-web-url"' --raw-output | sed 's/admin/hol_cicd/g'
```
...
1. Set the location of your wallet file.
   ```
   set cloudconfig /home/opc/wallet/Wallet_MyAtpDb.zip
   ```
...
```
CREATE USER hol_cicd IDENTIFIED BY HandsOnLabUser1;
ALTER USER hol_cicd TEMPORARY TABLESPACE temp;

GRANT CREATE SESSION, RESOURCE, UNLIMITED TABLESPACE TO hol_cicd;
...
BEGIN
 ords_admin.enable_schema(
  ...
  p_url_mapping_pattern => 'hol_cicd',
...
CREATE USER hol_cicd_good IDENTIFIED BY HandsOnLabUser1;
ALTER USER hol_cicd_good TEMPORARY TABLESPACE temp;

GRANT CREATE SESSION, RESOURCE, UNLIMITED TABLESPACE TO hol_cicd_good;
...
BEGIN
 ords_admin.enable_schema(
  p_enabled => TRUE,
  p_schema => 'hol_cicd_good',
  p_url_mapping_type => 'BASE_PATH',
  p_url_mapping_pattern => 'hol_cicd_good',
  p_auto_rest_auth => NULL
 );
 commit;
END;
...
-- Add table lb_groups
CREATE TABLE HOL_CICD_GOOD.customers (id INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL,
                                 email VARCHAR2(255),
...
CREATE TABLE HOL_CICD_GOOD.order_statuses (status VARCHAR2(10) NOT NULL,
                                      description VARCHAR2(255),
                                      CONSTRAINT order_statuses_pk PRIMARY KEY (status));

-- Add table lb_groups
CREATE TABLE HOL_CICD_GOOD.orders (id INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL,
                              order_datetime TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
                              customer_id INTEGER NOT NULL,
                              order_status VARCHAR2(10) NOT NULL,
                              CONSTRAINT orders_pk PRIMARY KEY (id),
                              CONSTRAINT orders_customer_id_fk FOREIGN KEY (customer_id) REFERENCES HOL_CICD_GOOD.CUSTOMERS(id),
                              CONSTRAINT order_status_fk FOREIGN KEY (order_status) REFERENCES HOL_CICD_GOOD.order_statuses (status));

-- Load Order_Statuses data
INSERT INTO HOL_CICD_GOOD.order_statuses (status, description) VALUES ('New', 'Still being created');

INSERT INTO HOL_CICD_GOOD.order_statuses (status, description) VALUES ('Submitted', 'Awaiting payment');

INSERT INTO HOL_CICD_GOOD.order_statuses (status, description) VALUES ('Shipped', 'Has been sent');

INSERT INTO HOL_CICD_GOOD.order_statuses (status, description) VALUES ('Complete', 'Has been delivered');

-- Create Database Lock Table
CREATE TABLE HOL_CICD_GOOD.DATABASECHANGELOGLOCK (ID INTEGER NOT NULL,
...
CREATE TABLE HOL_CICD_GOOD.DATABASECHANGELOG (ID VARCHAR2(255) NOT NULL,
